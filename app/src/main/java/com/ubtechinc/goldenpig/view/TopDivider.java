...
public class TopDivider extends RecyclerView.ItemDecoration {
    ...
    public TopDivider(Drawable divider, int orientation) {
        setDivider(divider);
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent,
        ...
        if (mOrientation == LinearLayoutManager.HORIZONTAL) {
            if(parent.getChildAdapterPosition(view) == 0){
                outRect.set(height, 0, leftMargin + width + rightMargin, 0);
            }else {
                outRect.set(0, 0, leftMargin + width + rightMargin, 0);
            }
        } else {
            if (parent.getChildAdapterPosition(view) == 0) {
                outRect.set(0, height, 0, topMargin + height + bottomMargin);
            } else {
